...
<template>
  <div>
    <h4 class="text-neutral-800 dark:text-neutral-200 text-xs font-semibold mb-3">
    ...
    <form @submit.prevent="handleSubmit" class="space-y-2">
      ...
      <div v-if="mode === 'register'" class="flex items-start gap-2 px-2 py-2 rounded bg-amber-500/10 border border-amber-500/20">
        ...
        <p class="text-[10px] text-amber-700 dark:text-amber-300/90 leading-relaxed">Si pierdes tu password, tus datos no se pueden recuperar. No existe mecanismo de recuperación en un sistema zero-knowledge.</p>
      </div>

      <p v-if="auth.authError.value" class="text-red-600 dark:text-red-400 text-xs">{{ auth.authError.value }}</p>
    ...
    <div class="mt-2 flex items-center justify-between">
      ...
      <router-link
        v-if="mode === 'login'"
        to="/forgot-password"
        class="text-[10px] text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300 transition-colors"
      >
