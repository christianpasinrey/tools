...
const emit = defineEmits(['drop', 'dragover', 'dragleave', 'click', 'toggle-select', 'reorder', 'preview'])
...
<template>
  ...
  >
    ...
    <div
      v-if="!hasFile"
      @click="emit('click')"
      :class="['group h-full flex flex-col items-center justify-center cursor-pointer transition-colors rounded-xl border-2 border-dashed bg-white/50 dark:bg-neutral-900/50', isDragging ? 'border-red-500/60 bg-red-500/5' : 'border-neutral-300 dark:border-neutral-700 hover:border-red-400/50 dark:hover:border-red-500/40 hover:bg-neutral-100/60 dark:hover:bg-neutral-800/40']"
    >
      <EditorEmptyState
        icon="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"
        title="Arrastra un PDF aquí"
        accent="#ef4444"
        :chips="['Combinar', 'Dividir', 'Rotar', 'Extraer']"
        formats="Archivos .pdf — todo se procesa en tu navegador"
      />
    </div>
    ...
    <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
      <div
        ...
        @dblclick.stop="emit('preview', index)"
        :class="[
      ...
      >
        ...
        <!-- Drag Handle -->
        <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity" v-if="page.rotation === 0">
        ...
        <div
          class="absolute inset-0 flex items-center justify-center bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"
        >
          <div class="flex flex-col items-center gap-1 text-white">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7" />
            </svg>
            <span class="text-[10px]">Doble clic</span>
          </div>
        </div>
      </div>
    </div>
